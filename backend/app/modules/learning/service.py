...
from app.modules.evaluations.enums import AttemptMode, InputMethod
...
def activate_up_to_bank_target(
    ...
    activated = 0
    while should_activate_next(_active_count(db, user_id), _has_unseen_text(db, user_id), target):
        candidate = _next_unseen_text(db, user_id)
...
def _has_unseen_text(db: Session, user_id: uuid.UUID) -> bool:
    return _next_unseen_text(db, user_id) is not None


def _next_unseen_text(db: Session, user_id: uuid.UUID) -> Text | None:
    return db.scalar(
        select(Text)
        .where(
